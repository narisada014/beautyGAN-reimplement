...
config.batch_size = 1 # original is 1
...
config.checkpoint = "" # どこから学習するかのチェックポイント
...
def merge_cfg_arg(config, args):
    ...
    config.is_added_lumi_L1 = args.lumiL1
    config.is_added_lumi_L1_to_his = args.hisAndLumiL1
    config.is_added_lumi_L2_to_his = args.hisAndLumiL2
    config.is_added_lumi_L1_lambda_5_to_his = args.hisAndLumiL1Lambda5
    config.is_added_l_hist = args.makeupHisAndLHis
    config.is_luminance_matching_to_fake = args.isLuminanceHisMatch
    config.is_HS_matching = args.isHSHisMatch

    config.is_HLS_match_all = args.isHLSMatch_All
    config.is_HS_match_skin_HLS_match_other = args.isHSHisMatch_Skin_HLSHisMatchOther
    config.is_AB_match_all = args.isABHisMatch_All
    config.is_AB_match_skin_Lab_match_other = args.isABHisMatch_Skin_LabHisMatchOther
    config.is_grad_loss = args.isGradLoss
    config.is_glcm_loss = args.isGLCMLoss
    config.is_glcm_loss_with_count = args.isGLCMLossWithCount
    config.is_glcm_to_org_loss = args.isGLCMToOrgLoss
    # print(config)
