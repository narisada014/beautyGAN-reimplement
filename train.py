...
def parse_args():
    ...
    parser.add_argument('--lumiL1', action='store_true', help='輝度値の損失誤差を使用するかどうか')
    parser.add_argument('--hisAndLumiL1', action='store_true', help='histogram matchingの結果と元画像に輝度値の損失誤差を追加で使用するかどうか')
    parser.add_argument('--hisAndLumiL1Lambda5', action='store_true', help='histogram matchingの結果と元画像に輝度値の損失誤差を追加で使用するかどうか')
    parser.add_argument('--hisAndLumiL2', action='store_true', help='histogram matchingの結果と元画像に輝度値の二乗損失誤差を追加で使用するかどうか')
    parser.add_argument('--makeupHisAndLHis', action='store_true')
    parser.add_argument('--isLuminanceHisMatch', action='store_true')
    parser.add_argument('--isHSHisMatch', action='store_true')
    
    parser.add_argument('--isHLSMatch_All', action='store_true')
    parser.add_argument('--isHSHisMatch_Skin_HLSHisMatchOther', action='store_true')
    parser.add_argument('--isABHisMatch_All', action='store_true')
    parser.add_argument('--isABHisMatch_Skin_LabHisMatchOther', action='store_true')

    parser.add_argument('--isGradLoss', action='store_true')

    parser.add_argument('--isGLCMLoss', action="store_true")
    parser.add_argument('--isGLCMLossWithCount', action="store_true")
    parser.add_argument('--isGLCMToOrgLoss', action="store_true")
